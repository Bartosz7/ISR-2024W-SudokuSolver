...
def augment_dataset(input_dir, output_dir="1000augmented_dataset"):
    ...
    def add_lines(draw, width, height):
        ...
        if line_type == 'left':
            ...
            if corner == 'tl':
                draw.line([(0, line_length), (0, 0), (line_length, 0)], fill='black', width=2)
            elif corner == 'tr':
                draw.line([(width-line_length, 0), (width, 0), (width, line_length)], fill='black', width=2)
            elif corner == 'bl':
                draw.line([(0, height-line_length), (0, height), (line_length, height)], fill='black', width=2)
            elif corner == 'br':
                draw.line([(width-line_length, height), (width, height), (width, height-line_length)], fill='black', width=2)

    def add_outline(draw, width, height):
        if random.random() < 0.5:  # 50% chance to add outline
            margin = random.randint(5, 20)
            draw.rectangle([(margin, margin), (width-margin, height-margin)], 
                         outline='black', width=random.randint(1, 3))
